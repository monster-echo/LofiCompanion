...
import { getBizApiBase } from '../../../data/apiClient';
import { createSseTransport } from '../data/sseTransport';
import { studyroomStreamUrl } from '../domain/streamUrl';
import {
...
/**
 * 自习室应用层 React 接线（对齐 FocusStore 模式）：Provider 把 SSE transport
 * （biz 同源 /api/studyroom/stream）、SecureStore token 注入
 * createStudyRoomController，经 useSyncExternalStore 把快照暴露给屏幕。
 * 屏幕生命周期由 useFocusEffect 驱动 actions.enter/leave（原生 Tab 保活，
 * 切 Tab 即断开省电）。
 */
...
export function StudyRoomProvider({
  ...
  const [controller] = useState<StudyRoomController>(() =>
    createStudyRoomController({
      ...
      resolveUrl: () => studyroomStreamUrl(getBizApiBase()),
