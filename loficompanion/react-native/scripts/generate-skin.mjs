 ...
 *
 * 用法：
 *   node scripts/generate-skin.mjs [slug]            # 生成（缺省 rainy-study-room）
 *   node scripts/generate-skin.mjs [slug] --check    # 校验生成物与 YAML 同步（CI）
 */
...
const slug = args.find((arg) => !arg.startsWith('--')) ?? 'rainy-study-room';
const skinDir = join(pkgRoot, 'assets', 'skins', slug);
const yamlPath = join(skinDir, 'skin.yaml');

function fail(message) {
  console.error(`generate-skin: ${message}`);
  process.exit(1);
}
