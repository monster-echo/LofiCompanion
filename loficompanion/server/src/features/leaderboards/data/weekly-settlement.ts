...
async function groupGoalMinutes(groupId: string): Promise<number> {
  const row = await database.prepare(
    'SELECT weekly_goal_minutes FROM study_groups WHERE id = ?',
  ).get(groupId) as { weekly_goal_minutes: number } | undefined;
...
async function grantWeeklyGroupPhoto(memberIds: ReadonlyArray<string>): Promise<number> {
  const roomItem = await database.prepare(
    'SELECT id FROM room_items WHERE item_id = ?',
  ).get(WEEKLY_GROUP_PHOTO_ITEM_ID) as { id: string } | undefined;
  if (!roomItem) return 0; // 防御：种子未生效时不发
  let granted = 0;
  for (const userId of memberIds) {
    const result = await database.prepare(
      `INSERT INTO user_room_items(user_id, room_item_id, source_grant_id, unlocked_at)
       VALUES (?, ?, NULL, ?) ON CONFLICT (user_id, room_item_id) DO NOTHING`,
    ).run(userId, roomItem.id, nowIso());
    granted += result.changes;
  }
  return granted;
}
