...
import { resolveProfiles } from '@/features/profiles/client';
import {
...
//   → 原子 upsert leaderboard_scores（复合主键 upsert）；
  ...
  return await getDb().focusSession.findMany({
    where: {
      ...
      ended_at: {
        ...
        lt: new Date(startMs + 7 * DAY_MS).toISOString(),
      },
    },
...
async function upsertScore(
  ...
  const row = await getDb().leaderboardScore.upsert({
    where: {
      user_id_scope_type_scope_id_week_id_rule_version: {
        user_id: userId,
  ...
  return row as LeaderboardScoreRow;
}
...
export async function rankFromScores(
  ...
  const [profiles, settings] = await Promise.all([
    resolveProfiles(ids),
    getDb().leaderboardSettings.findMany({
      where: { user_id: { in: ids } },
      select: { user_id: true, public_display: true, opted_out: true },
    }),
  ]);
  ...
  const staged = rows
    .map((row) => {
      ...
      return {
        ...
        avatarUrl: hidden ? null : profile?.avatarUrl ?? null,
...
export async function readSnapshot(
  ...
  const existing = await getDb().leaderboardSnapshot.findUnique({
    where: {
      scope_type_scope_id_week_id_rule_version: {
        scope_type: scopeType,
        scope_id: scopeId,
        week_id: weekId,
        rule_version: LEADERBOARD_RULE_VERSION,
...
export async function writeSnapshot(
  ...
  const result = await getDb().leaderboardSnapshot.createMany({
    data: [{
      id: randomUUID(),
      scope_type: scopeType,
      scope_id: scopeId,
      week_id: weekId,
      rankings: JSON.stringify(envelope),
      settled_at: nowIso(),
      rule_version: LEADERBOARD_RULE_VERSION,
    }],
    skipDuplicates: true,
...
export async function finalizeView(
  ...
  const profile = (await resolveProfiles([viewerId])).get(viewerId) ?? null;
...
async function loadFriendIds(userId: string): Promise<string[]> {
  const rows = await getDb().friendship.findMany({
