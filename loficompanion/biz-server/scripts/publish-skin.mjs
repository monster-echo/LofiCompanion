...
// ---- 1) 海报/视频直传：posterUrl/videoUrl 本地文件名 → objectKey ----
const contentTypeByExt = { '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.webp': 'image/webp', '.mp4': 'video/mp4' };
if (!DRY_RUN) {
  // 单资产直传：签名 → PUT → 返回含租户前缀的完整 objectKey
  const uploadAsset = async (localName, label) => {
    ...
    if (!contentType) {
      console.error(`不支持的${label}格式: ${file}`);
      process.exit(1);
    }
    ...
    if (!put.ok) throw new Error(`${label}上传失败 ${put.status}: ${objectKey}`);
    // sign.objectKey 已含 <appId>/<environment>/ 前缀（= /storage/urls 认可的完整 key）
    console.log(`  ↑ ${localName} → ${sign.objectKey}（${(fs.statSync(file).size / 1024).toFixed(0)}KB）`);
    return sign.objectKey;
  };
  const isRemoteKey = (value) => /^https?:/i.test(value) || value.startsWith('loficompanion/');
  for (const state of states) {
    if (typeof state.posterUrl === 'string' && !isRemoteKey(state.posterUrl)) {
      state.posterUrl = await uploadAsset(state.posterUrl, '海报');
    }
    // 视频可选：videoUrl 本地文件名（如 videos/ready.mp4，相对皮肤资产目录）
    if (typeof state.videoUrl === 'string' && !isRemoteKey(state.videoUrl)) {
      state.videoUrl = await uploadAsset(state.videoUrl, '视频');
    }
  }
} else {
  console.log('DRY_RUN：跳过海报/视频上传');
}
