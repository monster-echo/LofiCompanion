# ASC IAP Review Notes（深夜工作台 ¥6 单买）
...
This in-app purchase is a purely cosmetic companion skin ("Midnight Workstation" / 深夜工作台), a one-time non-consumable purchase (¥6) that permanently unlocks an alternative visual theme for the study companion. It provides no functional advantage.
...
3. Select "深夜工作台" (Midnight Workstation) — its card shows a ¥6 price badge.
4. On the detail page, tap "¥6 永久解锁" (Unlock forever), then "确认支付" (Confirm) — the Apple payment sheet appears and the purchase completes via App Store IAP.
...
该内购为纯装饰性陪伴皮肤「深夜工作台」：¥6 单买永久解锁，无任何功能优势。
...
- 提交前确认生产库 `skin_products`（midnight）已配 `store_product_ids` + `provider='store'`，否则审核时真实购买会 503。
